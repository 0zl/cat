...
{
   import Book_fla.Button_3;
   import Book_fla.mcBackbutton_75;
   import Book_fla.questList_173;
   ...
   public class theArchive extends MovieClip
   {
      ...
      public var txtZoneDesc:TextField;
      
      public var btnQuit:Button_3;
      
      public var txtZoneName:TextField;
      
      public var btTravel:SimpleButton;
      
      public var SBar:MovieClip;
      
      public var questList:questList_173;
      
      public var btnBack:mcBackbutton_75;
      ...
      {
         super();
         this.preview.imgMask.visible = false;
         this.preview.imgMaskMask.visible = false;
         this.btPreview.addEventListener(MouseEvent.CLICK,this.onPrev,false,0,true);
         this.preview.btnClose.addEventListener(MouseEvent.CLICK,this.onClose,false,0,true);
         this.btTravel.addEventListener(MouseEvent.CLICK,this.onTravel,false,0,true);
         this.btnQuit.addEventListener(MouseEvent.CLICK,this.onQuit,false,0,true);
         this.btnBack.addEventListener(MouseEvent.CLICK,this.onQuit,false,0,true);
         ...
         this.Len = questsGet.length;
         questsGet.sort(this.orderName);
         this.i = 0;
         while(this.i < this.Len)
         {
            this.quest = questsGet[this.i];
            this.questListItem = new listQuestItem();
            this.questListItem.QuestName.text = this.quest.strName;
            if(this.quest.strType != "M")
            {
               this.questListItem.MemberIcon.visible = false;
            }
            if(this.quest.strMap != null)
            {
               this.questListItem.strMap = this.quest.strMap;
               this.questListItem.strPrev = this.quest.strPrev;
               this.questListItem.strName = this.quest.strName;
               this.questListItem.strDesc = this.quest.strDesc;
               this.questListItem.addEventListener(MouseEvent.CLICK,this.onQuestsClick,false,0,true);
               this.questListItem.buttonMode = true;
            }
            this.questListItem.x = 400;
            this.questListItem.y = -35 + 70 * this.i;
            this.questList.addChild(this.questListItem);
            ++this.i;
         }
         this.questList.mask = this.cntMask;
         this.mDown = false;
         this.hRun = this.SBar.bar.b.height - this.SBar.h.height;
         this.dRun = this.questList.height - this.cntMask.height + 5;
         this.oy = this.questList.y;
         this.SBar.h.addEventListener(MouseEvent.MOUSE_DOWN,this.onScrDown,false,0,true);
         main._stage.addEventListener(MouseEvent.MOUSE_UP,this.onScrUp,false,0,true);
         main._stage.addEventListener(MouseEvent.MOUSE_WHEEL,this.onWheel,false,0,true);
         this.questList.addEventListener(Event.ENTER_FRAME,this.hEF,false,0,true);
         this.questList.addEventListener(Event.ENTER_FRAME,this.dEF,false,0,true);
         this.activeMap = questsGet[0].strMap;
         this.txtZoneName.text = questsGet[0].strName;
         this.txtZoneDesc.text = questsGet[0].strDesc;
         this.zPrev = questsGet[0].strPrev;
      }
      ...
      public function onPrev(e:MouseEvent) : void
      {
         this.preview.imgMask.visible = false;
         this.preview.imgMaskMask.visible = false;
         this.preview.imgMask.scaleX = this.preview.imgMask.scaleY = 1;
         var loader:Loader = new Loader();
         loader.contentLoaderInfo.addEventListener(Event.COMPLETE,this.onComplete);
         loader.load(new URLRequest(this.zPrev));
         this.preview.visible = true;
      }
      
      public function onTravel(e:MouseEvent) : void
      {
         main.Game.world.gotoTown(this.activeMap,"Enter","Spawn");
         main._stage.removeEventListener(MouseEvent.MOUSE_UP,this.onScrUp);
         ...
         this.questList.removeEventListener(Event.ENTER_FRAME,this.hEF);
         this.questList.removeEventListener(Event.ENTER_FRAME,this.dEF);
         this.parent.removeChild(this);
      }
      
      public function onClose(e:MouseEvent) : void
      {
         if(this.preview.imgMask.numChildren > 1)
         {
            this.preview.imgMask.removeChildAt(0);
         }
         this.preview.visible = false;
      }
      ...
      public function onQuestsClick(param1:MouseEvent) : void
      {
         ...
         this.activeMap = param1.currentTarget.strMap;
         this.txtZoneName.text = param1.currentTarget.strName;
         this.txtZoneDesc.text = param1.currentTarget.strDesc;
         this.zPrev = param1.currentTarget.strPrev;
      }
      
      function onComplete(e:Event) : void
      {
         var movie:* = e.currentTarget.content;
         this.preview.imgMask.addChild(movie);
         this.preview.imgMask.mask = this.preview.imgMaskMask;
         this.preview.imgMask.visible = true;
         this.preview.imgMaskMask.visible = true;
      }
      ...
      public function onWheel(e:MouseEvent) : void
      {
         ...
         _local2 = this.SBar;
         _local2.h.y = int(this.SBar.h.y) + e.delta * -1;
      ...
      public function dEF(_arg1:Event) : *
      {
         ...
         var _local3:* = this.questList;
         var _local4:* = -(_local2.h.y - 4.75) / this.hRun;
